...
| 3 | **`length_of_stay` must match `(departure_date - arrival_date)` in days** | Cross-validated at the ingestion edge (Python) and again in dbt silver. Mismatches are quarantined, not silently corrected. |
